...
assert_live_target_config() {
  ...
  live_image="$(jq -r '.image // empty' <<<"$container_json")"
  live_processing="$(
    jq -r \
      '[.env[]? | select(.name == "DIRECTIVE_PROCESSING_VERSION") | .value] |
...
revalidate_validation_evidence() {
  local canonical_record actual_digest evidence_digest
  assert_execution_image "$VALIDATE_EXECUTION"
  assert_execution_mode "$VALIDATE_EXECUTION" validate
  jq -c '.producer_record' "$VALIDATION_EVIDENCE_FILE" >"$VALIDATION_SUMMARY_FILE"
  validate_metadata_summary
...
if [[ "$PHASE" == publish || "$PHASE" == verify || "$PHASE" == audit ]]; then
  ...
  az acr build \
    ...
    --file setup/directive_ingest/Dockerfile \
    "$BUILD_CONTEXT"
  resolve_image_digest
fi
