...
if "validation_execution_id" in record or "verify_execution_id" in record:
...
if record.get("search_index") != search_index or record["environment"].get("search_index") != search_index:
...
if not _int(record.get("directive_count")) or not 0 <= record["directive_count"] <= 100:
...
if len(record["directive_version_ids"]) != record["directive_count"] or len(record["directive_version_ids"]) > 100:
    raise SystemExit("directive version IDs must match directive_count and remain bounded")
warnings = record.get("warnings")
if not isinstance(warnings, list) or len(warnings) > 100 or record.get("warning_count") != len(warnings):
