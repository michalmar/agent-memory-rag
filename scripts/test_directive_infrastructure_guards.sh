...
if grep -q 'search_index_exists "$V2_INDEX" && die' "$RESET_SCRIPT"; then
  echo "successful v2 index deletion still propagates a false status" >&2
  exit 1
fi
if awk '
  /^revalidate_validation_evidence\(\)/ { in_function = 1 }
  in_function { print }
  in_function && /^}/ { exit }
...
' "$PLAN" >"$BAD_PLAN"
directive_assert_cosmos_recreation_plan "$BAD_PLAN"
