...
recreate_cosmos_containers() {
  ...
  plan_args+=("-out=$COSMOS_PLAN_FILE")
  for address in \
    azurerm_cosmosdb_sql_container.directive_catalog \
    azurerm_cosmosdb_sql_container.directive_content \
    azurerm_cosmosdb_sql_container.directive_mandates; do
    if "${TERRAFORM_CMD[@]}" -chdir="$INFRA_DIR" state show "$address" \
      >/dev/null 2>&1; then
  ...
  "${TERRAFORM_CMD[@]}" -chdir="$INFRA_DIR" plan "${plan_args[@]}"
  ...
  directive_assert_cosmos_recreation_plan "$COSMOS_PLAN_JSON_FILE" || die \
    "Terraform plan is not an exact recreation plan for the three Cosmos containers"
...
search_resource_exists() {
  ...
  local error_file status
  case "$collection" in
    indexes|knowledgebases|knowledgesources) ;;
    *) die "Unsupported Search resource collection: $collection" ;;
  esac
  error_file="$(mktemp)"
  if "${AZ_CMD[@]}" rest \
    --method get \
    --url "https://${SEARCH_NAME}.search.windows.net/$collection/$name?api-version=$SEARCH_API_VERSION" \
  ...
  cat "$error_file" >&2
  rm -f "$error_file"
  die "Search resource existence check failed for $collection/$name (status $status)"
}

search_index_exists() {
  search_resource_exists indexes "$1"
}
...
delete_search_resource() {
  ...
  "${AZ_CMD[@]}" rest \
    ...
    --resource "https://search.azure.com" \
    --output none
  if search_resource_exists "$collection" "$name"; then
    die "Search resource $collection/$name still exists after delete"
  fi
}
