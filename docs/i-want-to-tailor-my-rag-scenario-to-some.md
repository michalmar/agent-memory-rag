...
Use the stable `directive_id` as the assignment key. This lets a mandatory assignment follow the directive when a new current version is published. Each environment creates the ignored `setup/directives/mandatory/mand.csv` from `mand.csv.example`; it is a headerless four-column mapping of `user_upn,entra_object_id,directive_id,flag`, where `flag` is `M` and rows represent only positive assignments. Never commit or share the target identity mapping. Normalize UPN casing and whitespace, validate the Entra object ID as a UUID, validate every flag and directive ID, require one consistent UPN-to-object-ID mapping within the file, and deduplicate exact repeated rows.
...
- **Retrieval planning:** the Hosted GPT-5.6 agent supplies final focused queries; the backend executes bounded direct hybrid Search requests concurrently and fuses their ranked results deterministically, with no second generative planner;
